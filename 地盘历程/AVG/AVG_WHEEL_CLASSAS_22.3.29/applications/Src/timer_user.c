...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if (htim->Instance == TIM1)
	{
		...
		{
				int16_t vtx=8*rc_ctrl.rc.ch[2];//遥杆值转化为地盘各方向速度值
				int16_t vty=8*rc_ctrl.rc.ch[3];
			  int16_t vtw=8*rc_ctrl.rc.ch[0];
			
			Speed_Motor_Target[0] = sqrt( pow( vty-0.7071*vtw,2)+pow(vtx-0.7071*vtw,2));//M3508的速度结算
		...
		{
		  Angle_target[0]=atan2(vty-0.7071*vtw ,vtx-0.7071*vtw);//M2006角度解算
      Angle_target[1]=atan2(vty-0.7071*vtw ,vtx+0.7071*vtw);
		  ...
		  for(int a=0;a<4; a++)    //for循环内 计算出M2006 pid位置环要转过的编码值
		  {
		   Angle_erra[a]=-1.570796+Angle_target[a];//
			 ...
			 Angle_Helm_Target[a]=Angle_erra_correct[a]*4367.18784*36;  	//  M2006 减速比36   4367.18784==8191*67/20/2/PI               
		  }
		}			
		else if(vtx==0&&vty==0&&vtw==0)//遥感回正时 M2006 pid 编码值为0
		{
		  for(int a=0;a<4; a++)
		  {
